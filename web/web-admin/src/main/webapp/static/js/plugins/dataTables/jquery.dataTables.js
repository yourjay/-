...
(/** @lends <global> */function( $ ) {
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @param {int} iCol column index.html to consider
	...
	/**
	 * Covert the index.html of a visible column to the index.html in the data array (take account
	 * of hidden columns)
	 *  @param {object} oSettings dataTables settings object
	 *  @param {int} iMatch Visible column index.html to lookup
	 *  @returns {int} i the data index.html
	 *  @memberof DataTable#oApi
	 */
	...
	/**
	 * Covert the index.html of an index.html in the data array and convert it to the visible
	 *   column index.html (take account of hidden columns)
	 *  @param {int} iMatch Column index.html to lookup
	 *  @param {object} oSettings dataTables settings object
	 *  @returns {int} i the data index.html
	 *  @memberof DataTable#oApi
	 */
	 ...
	 *    column index.html and the definition for that column.
	 *  @memberof DataTable#oApi
	 */
	 ...
	 *    if nTr is.
	 *  @returns {int} >=0 if successful (index.html of new aoData entry), -1 if failed
	 *  @memberof DataTable#oApi
	 */
	...
	/**
	 * Take a TR element and convert it to an index.html in aoData
	 *  @param {object} oSettings dataTables settings object
	 *  @param {node} n the TR element to find
	 *  @returns {int} index.html if the node is found, null if not
	 *  @memberof DataTable#oApi
	 */
	...
	/**
	 * Take a TD element and convert it into a column data index.html (not the visible index.html)
	 *  @param {object} oSettings dataTables settings object
	 *  @param {int} iRow The row number the TD/TH can be found in
	 *  @param {node} n The TD/TH element to find
	 *  @returns {int} index.html if the node is found, -1 if not
	 *  @memberof DataTable#oApi
	 */
	 ...
	 *  @param {object} oSettings dataTables settings object
	 *  @param {int} iRow aoData row id
	 *  @param {int} iCol Column index.html
	...
	function _fnClearTable( settings )
	{
	 ...
	 /**
	 * Take an array of integers (index.html array) and remove a target integer (value - not
	 * the key!)
	 ...
	 *
	 * @param {object} settings DataTables settings object
	 * @param  {int}    rowIdx   Row index.html to invalidate
	 * @memberof DataTable#oApi
	 *
	 ...
	 *  @param {int} colIdx column sorting index.html
	...
	DataTable = function( options )
	{
		 ...
		 *  @param {mixed} target The index.html of the row from aoData to be deleted, or
		 ...
		 *    data index.html for the row (see fnGetPosition) and the data for that row used.
		 *  @param {int} [col] Optional column index.html that you want the data of.
		 ...
		 * flexible.
		 *  @param {int} [iRow] Optional row index.html for the TR element you want
		 ...
		 * and column index.html including hidden columns
		 *  @param {node} node this can either be a TR, TD or TH in the table's body
		 *  @returns {int} If nNode is given as a TR, then a single index.html is returned, or
		 *    if given as a cell, an array of [row index.html, column index.html (visible),
		 *    column index.html (all)] is given.
		 *  @dtopt API
		 *  @deprecated Since v1.10
		 *
		 *  @example
		 ...
		 *  @param {int} iCol the data index.html to sort on. Note that this will not match the
		 *    'display index.html' if you have hidden data entries
		 *  @dtopt API
		 *  @deprecated Since v1.10
		 *
		 *  @example
		 ...
		 *  @param {node|int} mRow TR element you want to update or the aoData index.html
		...
		this.each(function() {
			...
			/* Copy the data index.html array */
	...
	{
		// Integer is used to pick out a table by index.html
	...
	/**
	 * Get the current page index.html.
	 *
	 * @return {integer} Current page index.html (zero based)
	 *//**
	 ...
	 *  * `integer` - The page index.html to jump to
	 ...
	 *  * `page` - Current page index.html (zero based - i.e. the first page is `0`)
	 *  * `pages` - Total number of pages
	 *  * `start` - Display index.html for the first record shown on the current page
	 *  * `end` - Display index.html for the last record shown on the current page
	...
	var _selector_row_indexes = function ( settings, opts )
	{
		...
		var
			...
			order  = opts.order,   // applied, current, index.html (original - compatibility with 1.9)
	...
	var __row_selector = function ( settings, selector, opts )
	{
		return _selector_run( selector, function ( sel ) {
			...
			// all records, so no need to check if the index.html is in there, since it
			// must be - dev error if the index.html doesn't exist).
			if ( selInt !== null && ! opts ) {
				return [ selInt ];
			}
		...
		if ( _pluck( settings.aoData, '_details' ).length > 0 ) {
			...
			table.on('draw.DT_details', function () {
				table.find('tbody tr').each( function () {
					// Look up the row index.html for each row and append open row
	 ...
	 * Columns
	 *
	 * {integer}           - column index.html (>=0 count from left, <0 count from right)
	 * "{integer}:visIdx"  - visible column index.html (i.e. translate to column index.html)  (>=0 count from left, <0 count from right)
				...
				if ( match ) {
					switch( match[2] ) {
						...
						case 'visible':
							...
							// Visible index.html given, convert to column index.html
							if ( idx < 0 ) {
						...
						case 'name':
							// match by name. `names` is column index.html complete and in order
					...
					return $( nodes )
						.filter( s )
						.map( function () {
							return $.inArray( this, nodes ); // `nodes` is column index.html complete and in order
						} )
						.toArray();
				}
			}
	...
	// Convert from one column index.html type, to another type
	 ...
	 *   the column index.html that the sorting condition applies to, the second is the
	 *   direction of the sort (`desc` or `asc`) and, optionally, the third is the
	 *   index.html of the sorting order from the `column.sorting` initialisation array.
	 *//**
	 * Set the ordering for the table.
	 *
	 * @param {integer} order Column index.html to sort upon.
	...
	DataTable.models.oRow = {
		 ...
		 * number of columns (although each index.html is generated only as it is
	...
	DataTable.models.oColumn = {
		 ...
		 * Sort it again (click again) and it will move on to the next index.html.
		 * Repeat until loop.
		 *  @type array
		 */
		"asSorting": null,
		 ...
		 *  @param {int} iRow The row index.html for the aoData data store
		 *  @default null
		 */
		"fnCreatedCell": null,
		 ...
		 * by index.html (needs a lookup to work by name).
		 *  @type string
		 */
		"sName": null,
	...
	DataTable.defaults = {
		 ...
		 * the column's index.html and a direction string ('asc' or 'desc').
		 *  @type array
		 *  @default [[0,'asc']]
		 *
		 *  @dtopt Option
		 ...
		 *     <li>0 or a positive integer - column index.html counting from the left</li>
		 *     <li>a negative integer - column index.html counting from the right</li>
		 ...
		 *  @param {int} dataIndex The index.html of this row in the internal aoData array
		 *
		 *  @dtopt Callbacks
		 ...
		 *          tfoot.getElementsByTagName('th')[0].innerHTML = "Starting index.html is "+start;
		 *        }
		 *      } );
		 *    } )
		 */
		 ...
		 *  @param {node} row "TR" element for the current row
		 *  @param {array} data Raw data array for this row
		 *  @param {int} displayIndex The display index.html for the current table draw
		 *  @param {int} displayIndexFull The index.html of the data in the full list of
		...
		"oLanguage": {
			 ...
			 *
			 * * `\_START\_` - Display index.html of the first record on the current page
			 * * `\_END\_` - Display index.html of the last record on the current page
	...
	DataTable.defaults.column = {
		 ...
		 * two columns.
		 *  @type array|int
		 *  @default null <i>Takes the value of the column index.html automatically</i>
		 *
		 ...
		 *  @param {int} row The row index.html for the aoData data store
		 *  @param {int} col The column index.html for aoColumns
		 *
		 ...
		 *
		 * * `integer` - treated as an array index.html for the data source. This is the
		 ...
		 *  @default null <i>Use automatically calculated column index.html</i>
		 *
	...
	DataTable.models.oSettings = {
		...
		/**
		 * Draw index.html (iDraw) of the last error when parsing the returned data
		 *  @type int
		 *  @default -1
		 */
		"iDrawError": -1,
		...
		/**
		 * Paging start point - aiDisplay index.html
		 *  @type int
		 *  @default 0
		 */
		"_iDisplayStart": 0,
		...
		/**
		 * Get the display end point - aiDisplay index.html
		 *  @type function
		 */
	...
	DataTable.ext = _ext = {
		 ...
		 * 3. `{int}` Row index.html ({@link DataTable.models.oSettings.aoData}), which
		 ...
		 *    // fourth column (i.e. the data[3] index.html) based on two input values
		 ...
		 *
		 * 1. `{int} page` The current page index.html
		 ...
		 * * `{int}` - Show page of the index.html given
		 ...
		 * array. The data in the array much be in the index.html order of the rows in
		 ...
		 * 1. `{object}` DataTables settings object: see
		 *    {@link DataTable.models.oSettings}
		 * 2. `{int}` Target column index.html
		 *
		 ...
		 *    {
		 *      return this.api().column( col, {order:'index.html'} ).nodes().map( function ( td, i ) {
		 *        return $('input', td).val();
		 *      } );
		 *    }
		 */
		...
		/**
		 * Index for what 'this' index.html API functions should use
		 *  @type int
		 *  @deprecated Since v1.10
		 */
		iApiIndex: 0,
	 ...
	 *  @param {event} e jQuery event object
	 *  @param {object} o DataTables settings object {@link DataTable.models.oSettings}
	 *  @param {int} column Column index.html
